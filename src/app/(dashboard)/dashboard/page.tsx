...
export default function DashboardPage() {
  const { email } = useUser();
  const { getSummaryStats, getModuleStatuses, isLoading } = useProgress();
  ...
  // Find the current in-progress module (if any)
  const statuses = getModuleStatuses();
  const inProgressModule = Object.entries(statuses).find(
    ([_, status]) => status === "in_progress",
  );
  const currentModuleId = inProgressModule ? inProgressModule[0] : undefined;
  ...
  return (
    <div className="space-y-6">
      ...
      <WelcomeBanner
        ...
        hasAnyProgress={stats.tasksCompleted > 0}
      />
